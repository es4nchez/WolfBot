...
class IntraAPIClient(object):
    ...
    def __init__(self, progress_bar=False):
        ...
        with open('config/secrets.yml', 'r') as cfg_stream:
            config = yaml.load(cfg_stream, Loader=yaml.BaseLoader)
            self.client_id = config['intra42']['CLIENT']
            self.client_secret = config['intra42']['SECRET']
            self.token_url = config['intra42']['URI']
            self.api_url = config['intra42']['ENDPOINT']
            self.scopes = config['intra42']['SCOPES']
